...
Each number (octet) can range from 0 to 255 (8 bits). This gives theoretically about **4.29 billion** unique addresses — a number that has been exhausted, which is one of the reasons for IPv6.
...
The **subnet mask** determines the boundary between the parts. The mask is always contiguous 1s followed by 0s in binary form.
...
Scenario: You have the network `192.168.1.0/24` (254 addresses) and need four separate subnets for: Employees, Guest, Servers, Admin.
...
A VLAN (Virtual Local Area Network) is a logical division of a physical network. With VLANs, you can have three separate networks on one physical switch — devices in each VLAN can see each other, but not devices in other VLANs (unless you explicitly allow it through a router).
...
VLAN IDs 0 and 4095 are reserved; usable VLAN IDs are 1–4094. **IEEE 802.1Q** is the international standard for VLAN tagging in Ethernet frames — it enables multiple logical networks on a single physical connection.
...
A PC on an access port doesn't "know" it's on a VLAN — it just sees a regular network. The tag is added by the switch and removed on the receiving end.

A **trunk port** is a switch port configured to carry traffic from multiple VLANs simultaneously by keeping the VLAN tags in the data packet.

**Native VLAN** is the VLAN that receives *untagged* traffic on a trunk port (default is VLAN 1). It is recommended to change the native VLAN from 1 for security reasons.

#### Why Segment with VLANs?

**Security**: A threat in the guest VLAN cannot reach servers in the server VLAN without passing through a router/firewall with explicit allow rules.

**Performance**: Each VLAN is its own broadcast domain. Broadcast traffic (ARP, DHCP Discover) is only sent within the VLAN — not to all devices in the entire network.

**Flexibility**: An employee who moves to another room only needs the port VLAN assignment changed on the switch — no new cabling.
...
Subnetting divides an IP address space into smaller, isolated networks. /24 is the most common LAN size (254 hosts). Understand the difference between network address, broadcast address, and usable host addresses. Remember the formula: 2^(host bits) − 2.
...
Subnetting (layer 3) divides the IP address space and is a routable separation. VLAN (layer 2) creates logical networks on a single physical infrastructure without requiring separate physical cables. The two are commonly used together: each VLAN gets its own subnet.

**Can devices in different VLANs communicate?**
Yes, but only via a router or layer 3 switch with explicit allow rules. This gives controlled traffic between segments — e.g., servers are accessible from the employee VLAN but not from the guest VLAN.

**What happens to broadcast traffic in a VLAN?**
Broadcasts (e.g. DHCP Discover, ARP) are only sent within the same VLAN. This means a large company with 10 VLANs has much lower broadcast load than if all devices were in one large flat network.
...
**What is IEEE 802.1Q?**
IEEE 802.1Q is the international standard for VLAN tagging in Ethernet frames. The tag is 4 bytes and contains, among other things, a 12-bit VLAN ID enabling 4094 different VLANs.

**What is CIDR and why is it better than classic subnetting?**
CIDR (Classless Inter-Domain Routing) allows you to use subnet masks that don't follow the old class A/B/C boundaries. This gives much more flexible addressing and is the foundation of all modern IP planning.

**Can you have VLANs on wireless networks?**
Yes. Modern access points support SSID-to-VLAN mapping. The guest WiFi network can be connected to VLAN 20, while employee WiFi connects to VLAN 10 — the traffic is kept separate even though both use the same physical access point.
...
**Answer:** An access port belongs to one VLAN and sends untagged traffic to end devices. A trunk port carries tagged traffic for multiple VLANs and is used between switches and to routers.
</details>

<details>
<summary>Question 3: Which IEEE standard defines VLAN tagging?</summary>

**Answer:** IEEE 802.1Q defines VLAN tagging in Ethernet frames. The tag is 4 bytes and contains, among other things, a 12-bit VLAN ID (1–4094).
</details>

<details>
<summary>Question 4: Why is VLAN useful for network security?</summary>

**Answer:** VLANs create separate broadcast domains. A device in one VLAN cannot communicate directly with devices in another VLAN without passing through a router or firewall with explicit allow rules. This limits the spread of threats and prevents unauthorized access between networks.
</details>
