...
The physical layer defines the electrical, optical, and wireless signals that represent bits (0 and 1). It includes connector types (RJ-45), cable types (Cat5e, Cat6, fiber), and transmission methods. Nothing here knows about addresses or logical structure — it's just bits.
...
Introduces logical addressing (IP addresses) and routing between networks. A router operates at layer 3. The network layer determines the best path from source to destination through potentially many intermediate networks.

**Layer 4 — Transport Layer**
Segments data from the application layer and handles end-to-end communication. TCP provides reliable delivery with sequence numbers and acknowledgments (ACK). UDP is fast and simple without such guarantees. Port numbers identify which application the traffic belongs to.

**Layer 5 — Session Layer**
Manages *sessions* — persistent connections between applications. The session layer establishes, maintains, and terminates the dialogue between two systems. In practice, much of this functionality is integrated into application protocols in TCP/IP.
...
The top layer is what the user interacts with. This is where protocols that applications use directly reside: HTTP for web browsing, SMTP for sending email, FTP for file transfer, DNS for name resolution.

### Encapsulation and PDU

When data is sent down through the OSI layers, each layer adds its own control information (header). This process is called **encapsulation**. Each layer works with its own **PDU (Protocol Data Unit)**: at layer 4 it's called a *Segment*, at layer 3 a *Packet*, and at layer 2 a *Frame*. At the receiving end, it's unpacked in reverse order — each layer reads and removes its header.
...
OSI layers 5, 6, and 7 together correspond to the application layer in TCP/IP. In practice, the functions of the presentation and session layers are implemented directly in application protocols or in TLS.
...
OSI gives us a precise, shared reference language for talking about network problems and protocols. When a technician says "the problem is at layer 2," everyone understands what that means — even if the network is running TCP/IP.

**What is the difference between layer 3 and layer 2 addresses?**
Layer 2 uses MAC addresses (physical, hard-coded) for communication within one network. Layer 3 uses IP addresses (logical, configurable) for communication between different networks.

**What happens at layer 6 (Presentation) in practice?**
Encryption via TLS/SSL is the most common example. When you visit an HTTPS site, layer 6 encrypts the data. Compression (gzip) and character conversion (UTF-8) are other examples.

**Can a single piece of hardware operate at multiple layers?**
Yes. A "layer 3 switch" operates at both layer 2 and layer 3 — it can route between VLANs in addition to functioning as a regular switch. Firewalls often operate at layers 3–7.

**What is a PDU, and why is the term useful?**
PDU (Protocol Data Unit) is the name for the data unit at a given layer: Segment (layer 4), Packet (layer 3), Frame (layer 2), Bit (layer 1). It makes it easier to describe exactly what is being processed at each step.

**What is encapsulation?**
Encapsulation is the process where data is wrapped in control information (headers) from each layer of the OSI model as it travels down the stack. At the receiving end, it's unwrapped layer by layer.

**Why is the session layer (layer 5) important?**
The session layer manages the dialogue between applications — who speaks, in what order, and how to end cleanly. In practice, this is often integrated into TLS and application protocols.

**How do I actively use OSI in troubleshooting?**
Start at layer 1 and work up: check cable → switch → IP address → firewall ports → application settings. Or start at layer 7 and work down if you know the network is working.
...
**Answer:** Layers 5 (session), 6 (presentation), and 7 (application) in OSI together correspond to the application layer in the TCP/IP model.
</details>

<details>
<summary>Question 2: What is the task of the presentation layer (layer 6)?</summary>

**Answer:** The presentation layer handles formatting, encryption/decryption, and compression of data. TLS/SSL, which encrypts HTTP traffic to HTTPS, is an example of functionality from this layer.
</details>

<details>
<summary>Question 3: What is the difference between a switch and a router from the OSI model perspective?</summary>

**Answer:** A switch primarily operates at layer 2 (data link layer) and uses MAC addresses to forward frames within one network. A router operates at layer 3 (network layer) and uses IP addresses to route packets between different networks.
</details>

<details>
<summary>Question 4: What is the session layer (layer 5) responsible for?</summary>

**Answer:** The session layer manages the creation, maintenance, and termination of sessions (persistent connections) between applications on two systems.
</details>
