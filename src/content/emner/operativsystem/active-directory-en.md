...
**Forest**
The topmost level in AD. A forest can contain one or more trees. All domains in the same forest share a common schema and global catalog. `Enterprise Admins` have control over the entire forest.

**Tree**
A collection of domains with a contiguous DNS namespace. Example: `school.no` with the subdomain `students.school.no`.

**Domain**
The fundamental unit in AD. A domain is a logical group of users, computers, and resources under common administration. It is identified by a DNS name, e.g., `school.local` or `company.no`.
...
The first DC in a domain is also called the first domain-creating controller and holds special FSMO roles (*Flexible Single Master Operation*).

### LDAP

**LDAP** (Lightweight Directory Access Protocol) is the protocol AD uses to provide access to directory data. When you search for a user in AD or an application authenticates against AD, LDAP is used. Encrypted LDAP is called LDAPS (LDAP over SSL/TLS, port 636).
...
Kerberos is more secure than NTLM because the password is never transmitted, only encrypted tickets with a limited lifespan.
...
The default `Users` container holds the built-in accounts: `Administrator`, `Guest`, and `KRBTGT`. It is best practice to create your own OUs and move accounts there.
...
GPOs are inherited through the hierarchy (forest → domain → OU). A GPO linked to an OU overrides a GPO at the domain level.
...
At least one domain controller should have the Global Catalog role enabled. In a single-domain environment (like most schools), this is automatic.
...
AD DS is the classic on-premises directory service that requires a Windows Server and domain controller in your own network. Azure AD (now called Microsoft Entra ID) is Microsoft's cloud-based directory service for Microsoft 365 and cloud services. They can be synchronized via Azure AD Connect.

**Can a user log on to any machine in the domain?**
Yes — that is the whole point of a domain. The domain user exists in AD, not on the individual machine. GPOs can still restrict which users can log on to specific machines.

**What happens if the domain controller goes down?**
With a single DC, no one will be able to log on with domain accounts. Users already logged on can continue working locally for a while (cached credentials). That is why you should always have at least two DCs.

**What is the difference between Domain Admins and Enterprise Admins?**
Domain Admins have full control within one domain. Enterprise Admins have control over the entire AD forest (all domains). Enterprise Admins should only be used for forest-level operations.

**Why should I use groups instead of assigning access directly to users?**
With groups, you only need to change group membership when a user changes roles — not go through all resources manually. It is easier to audit (who has access to what) and reduces the risk of errors.

**What are FSMO roles and why are they important?**
Flexible Single Master Operation roles are specialized AD tasks that only one DC can perform at a time (e.g., issuing new SIDs). If the DC holding an FSMO role fails, certain operations can stop until the role is transferred to another DC.

**Can I change the domain name after installation?**
Technically yes, but it is complicated and risky. All GPOs, profiles, and service connections are tied to the domain name. It is recommended to plan the domain name thoroughly before installation.
...
**Answer:** A domain controller is a server with Active Directory Domain Services (AD DS) installed. It authenticates all logons in the domain, stores the AD database, and distributes group policies.

</details>

<details><summary>Question 2: What is the difference between an OU and a security group?</summary>

**Answer:** An OU (Organizational Unit) is a logical container for organizing objects in AD and linking GPOs to. A security group gathers users to assign them common access rights and permissions. OUs are used for administration; groups are used for access control.

</details>

<details><summary>Question 3: What is a GPO and what can it be used for?</summary>

**Answer:** A Group Policy Object is a set of settings automatically distributed to users and machines in AD. It can control password policies, security settings, software distribution, network drives, and much more.

</details>

<details><summary>Question 4: Why should you have at least two domain controllers?</summary>

**Answer:** For redundancy. If one domain controller fails, the other can continue authenticating users without downtime. With only one DC, the entire domain will stop working in the event of a failure.
